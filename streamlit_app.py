...
1. 快速分析词语「{word}」的语法特征
2. 参考以下核心规则（“核心规则采用自适应阈值机制：
若该词类存在 match_score ≥ 20 的规则，则仅展示这些规则作为核心规则；
若该词类所有规则的最高 match_score < 20，则将阈值降低为该词类内部的最高分，并选取该最高分及其排名前3的规则作为核心规则展示。
所有词类均可进入候选集合。”），筛选出最可能的10个候选词类：
{core_rules_text}
3. 说明筛选理由

#### 步骤2：对候选词类进行完整评分
1. 针对步骤1筛选出的候选词类，使用该词类的全部规则进行评分（符合规则填对应match_score，否则填mismatch_score）
