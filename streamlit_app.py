...
CODE_VERSION = "2026-09-14-cohere-adapt-original-data"
...
def _extract_cohere_v2_text(body: Dict[str, Any]) -> str:
    """Cohere V2 返回的文本提取；不改变后续原有 JSON 解析。"""
        ...
        try:
            if api_style == "gemini_native":
                ...
                if text_out:
                    if streaming_placeholder is not None:
                        streaming_placeholder.empty()
                    # 关键：转成你原程序已有的 OpenAI-like 结构，
                    # 后面 extract_text_from_response / extract_json_from_text 完全不动。
                    normalized = {
                        "choices": [
