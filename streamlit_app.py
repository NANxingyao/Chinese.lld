...
def ask_model_for_pos_and_scores(word: str, provider: str, model: str, api_key: str) -> Tuple[Dict[str, Dict[str, int]], str, str, str]:
    ...
    # ✨ 新方式：核心规则不再按 ≥20，而是展示每个词类得分最高的 3–5 条规则
    core_rules_text = "{\n"
    for pos, rules in RULE_SETS.items():
        # 将规则按 match_score 从高到低排序
        sorted_rules = sorted(rules, key=lambda x: x["match_score"], reverse=True)
        top_rules = sorted_rules[:5]  # 取前 5 条作为核心规则
        rule_str = ", ".join([f'"{r["name"]}": {r["match_score"]}' for r in top_rules])
        core_rules_text += f'  "{pos}": {{ {rule_str} }},\n'
    core_rules_text += "}"
    ...
    # ✨【跨模型通用】三层思维链 prompt（豆包/千问/DeepSeek 均适配）
    system_msg = f"""
你是一位中文语言学专家，需要对词语「{word}」进行词类隶属度分析。请严格按照以下步骤执行：

===========================
【步骤1：语法特征分析 + 粗筛】
===========================
1. 分析词语「{word}」的语法特征（如是否能作谓语、可否被程度副词修饰、是否表示动作/状态/性质、能否带宾语、能否作定语/状语等）。
2. 将这些语法特征与所有词类进行粗粒度比对。
3. 按“可能性”从高到低筛选出 **最可能的10个词类**（不足10个按实际数量）。
4. 必须说明：
   - 为什么这些词类进入候选集合
   - 为什么其他词类不进入候选集合

以下为各词类的“典型代表性规则”（由系统以最高分方式提供，仅用于步骤1参考）：
{core_rules_text}

⚠️ 注意：步骤1 不得进行打分，只能做语言学判断。

===========================
【步骤2：选取候选词类的核心规则】
===========================
对步骤1中的每个候选词类：
1. 在该词类的完整规则中自行挑选 3–5 条最能体现词类典型语法性质的规则作为“核心规则”。
2. 不限制分值（即使最高只有10分也可入选）。
3. 这些规则用于辅助解释，不会直接用于打分。

===========================
【步骤3：对候选词类进行严格计分】
===========================
1. 针对步骤1筛出的候选词类，使用该词类的全部规则进行严格评分。
2. 每条规则必须使用：
   - match_score（符合）
   - mismatch_score（不符合，必须是负分，不得用 0 代替）
3. 可参考以下完整规则（必须用于本步骤评分）：
"""
...
【步骤4：返回最终 JSON 结果】
===========================
最终输出必须是 JSON，不包含任何思考过程，格式如下：

{{
  "predicted_pos": "词类名称（从候选词类中选择）",
  "scores": {{
    "词类A": {{"规则1": 得分, "规则2": 得分, ...}},
    "词类B": {{"规则1": 得分, "规则2": 得分, ...}}
  }},
  "explanation": "说明为何最终选择该词类"
}}

⚠️ 要求：
- 必须按照规则提供的分值进行计算
- 必须输出完整 JSON，不能包含额外内容
- 若得分低的词类仍在候选，也必须保留评分结构
- 若候选词类不足10个，按实际数量；若所有词类都低分，保留评分最高的5个
...
"""
    ...
    return call_llm(provider, model, api_key, system_msg, user_prompt)
