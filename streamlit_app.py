...
def get_desktop_path():
    """获取用户桌面路径"""
    if platform.system() in ["Windows", "Darwin"]:  # Windows和macOS
        return os.path.join(os.path.expanduser("~"), "Desktop")
    else:  # Linux等其他系统
        return os.path.expanduser("~")  #  fallback到用户主目录

def save_results_to_single_excel(word, membership, detailed_scores_df, top_10_positions, predicted_pos, explanation):
    """将所有结果保存到桌面的单一Excel文件中"""
    try:
        # 获取桌面路径和文件名
        desktop = get_desktop_path()
        filename = "汉语词类隶属度测试结果.xlsx"
        ...
        # 准备隶属度数据
        membership_data = [{"词类": k, "隶属度": f"{v:.2%}"} for k, v in membership.items()]
        
        # 准备Top10数据
        top10_data = [{"词类": item[0], "隶属度": f"{item[1]:.2%}"} for item in top_10_positions]
        
        # 检查文件是否存在
        file_exists = os.path.exists(filepath)
        
        # 创建Excel写入器
        with pd.ExcelWriter(filepath, engine='openpyxl', mode='a' if file_exists else 'w', if_sheet_exists='replace' if file_exists else None) as writer:
            # 1. 保存基本信息（每次覆盖）
            ...
            membership_df.to_excel(writer, sheet_name="隶属度结果", index=False)
            
            # 3. 保存Top 10词类（每次覆盖）
            top10_df = pd.DataFrame(top10_data)
            top10_df.to_excel(writer, sheet_name="Top 10 词类", index=False)
            
            # 4. 保存详细得分（每次覆盖）
            detailed_scores_df.to_excel(writer, sheet_name="详细得分", index=False)
            
            # 5. 追加历史记录（新增工作表，保留所有历史）
            history_data = {
                "分析词语": word,
                "最可能词类": predicted_pos,
                "分析时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            if file_exists:
                ...
                if "历史记录" in book.sheetnames:
                    existing_history = pd.read_excel(filepath, sheet_name="历史记录")
