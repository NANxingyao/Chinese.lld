...
def ask_model_for_pos_and_scores(word: str, provider: str, model: str, api_key: str) -> Tuple[Dict[str, Dict[str, int]], str, str, str]:
    ...
    # 核心规则：不再用 ≥20 阈值，而是为每个词类选取若干条 match_score 最高的规则
    core_rules_text = "{\n"
    for pos, rules in RULE_SETS.items():
        # 按 match_score 从高到低排序
        sorted_rules = sorted(rules, key=lambda r: r["match_score"], reverse=True)
        # 取前 5 条作为该词类的代表性核心规则（如果规则少于 5 条，就全取）
        top_rules = sorted_rules[:5]
        inner = ", ".join([f'"{r["name"]}": {r["match_score"]}' for r in top_rules])
        core_rules_text += f'  "{pos}": {{ {inner} }},\n'
    core_rules_text += "}"

    # 完整规则：用于严格计分（match_score / mismatch_score 由规则系统定义，你在别处维护）
    full_rules_by_pos = {
        pos: "\n".join([f'"{r["name"]}": {r["match_score"]}' for r in rules])
        for pos, rules in RULE_SETS.items()
    }

    # 分阶段提示词，引入稳定的思维链：先语法特征 + 粗筛，再完整计分，最后只输出 JSON
    system_msg = f"""你是一位中文语言学专家，你的任务是根据提供的规则，为给定的词语「{word}」进行词类隶属度评分。请严格按照下面步骤执行，不要跳过或合并步骤：

===========================
【步骤1：语法特征分析与候选词类粗筛】
===========================
1. 分析词语「{word}」的语法特征，例如：
   - 是否能单独作谓语
   - 是否表示动作、状态、性质、事物等
   - 是否可以被程度副词（如“很、非常、比较”）修饰
   - 是否能带宾语
   - 是否可以作定语、状语、补语
   - 是否有明显的构式偏向（如“很+X”、“X+得+很+Adj”、“X+地+V”等）
2. 将这些语法特征与所有词类进行粗粒度比对。
3. 参考下方各词类的代表性核心规则，判断哪些词类与「{word}」具有较高的潜在匹配度，从中筛选出最可能的 10 个候选词类（若不足 10 个则按实际数量；若所有词类都很不匹配，也至少保留 5 个最可能的）。
4. 在你的思考过程中说明：
   - 为什么这些词类进入候选集合
   - 为什么其他词类未进入候选集合
（这些理由只保留在你的内部思考中，不出现在最终 JSON 中。）

以下为各词类的代表性核心规则（每个词类选取若干条 match_score 最高的规则，仅供步骤1参考）：
{core_rules_text}
...
===========================
对于步骤1筛选出的每一个候选词类：
1. 在该词类的全部规则中，自行挑选 3–5 条最能体现该词类典型语法特征的规则，作为该词类的“核心规则”。
2. 这些核心规则不要求 match_score ≥ 某个阈值，即使该词类最高分只有 10 分，也可以作为核心规则。
3. 这些核心规则用于帮助你在内部推理中把握每个词类的特征，但正式计分时必须使用全部规则。

===========================
【步骤3：对候选词类进行严格计分】
===========================
1. 仅针对步骤1中筛选出的候选词类进行计分。
2. 对每个候选词类，使用该词类的全部规则逐条判断「{word}」是否符合：
   - 若符合规则条件，记为该规则的 match_score；
   - 若不符合规则条件，记为该规则的 mismatch_score。
3. 必须严格使用规则中定义的分数：
   - 不符合规则时必须使用负分的 mismatch_score（如 -20、-10），绝对不能用 0 分代替。
   - 不得擅自修改任何 match_score 或 mismatch_score 的数值。
4. 你可以使用以下完整规则信息进行计分（只能使用这里提供的规则名称及其分值）：
"""

    # 拼接所有词类的完整规则（供模型在步骤3使用）
...
【步骤4：返回最终结果（仅输出 JSON）】
===========================
请根据候选词类的总得分，选择总分最高的词类作为最终预测词类，并严格按照以下 JSON 格式返回结果，不要包含任何额外文字或说明：

{
  ...
  "explanation": "简要说明判定为最可能词类的主要依据"
}

关键要求：
1. 最终回答中只允许出现上述 JSON 结构，不得出现其他文字。
2. "scores" 中的规则名称必须与提供的规则名称完全一致。
3. 若候选词类不足 10 个，按实际数量评分；若所有词类得分都很低，也至少保留得分最高的 5 个词类进行输出。
4. 对于不符合的规则，必须使用负的 mismatch_score，禁止使用 0 分或省略该规则。
"""

    # 用户提示：触发模型开始按上述步骤进行内部分析，并仅输出 JSON
    user_prompt = f"请根据上述步骤，为词语「{word}」进行词类隶属度评分并返回 JSON 结果。"

    # 这里假定你的 call_llm 接口签名为：call_llm(provider, model, api_key, messages)
    messages = [
